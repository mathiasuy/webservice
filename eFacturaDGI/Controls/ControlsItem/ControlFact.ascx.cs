...
{
    public partial class ControlFact : System.Web.UI.UserControl
    {
        ...
        public List<Item_Det_Fact> Items
        {
            get
            {
                return (List<Item_Det_Fact>)Session["ItemsFact"];
            }
        }
        ...
        protected void btnAdd(object sender, EventArgs e)
        {
            ...
            if (btnAdd != null)
            {
                ...
                Item_Det_Fact Item = new Item_Det_Fact(((List<Item_Det_Fact>)Session["ItemsFact"]).Count+1, CodItem, IndFact, NomItem, DscItem, Cantidad, UniMed, PrecioUnitario);

                ((List<Item_Det_Fact>)Session["ItemsFact"]).Add(Item);
                BindData();
            }
        }
        ...
        {
                GridView1.DataSource = ((List<Item_Det_Fact>)Session["ItemsFact"]);
                GridView1.DataBind();
                TextBox txtCodItem = GridView1.FooterRow.FindControl("txtCodItem") as TextBox;
                if (txtCodItem != null)
                {
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                int cont = 0;
                foreach (Item_Det_Fact t in (List<Item_Det_Fact>)Session["ItemsFact"])
                {
                    cont++;
                    t.NroLinDet = cont;
                }
        ...
        {
            ((List<Item_Det_Fact>)Session["ItemsFact"]).RemoveAt(e.RowIndex);
            BindData();
        }
